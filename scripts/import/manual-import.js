/**
 * Manual import script for documentary data from combatfilms.com
 * 
 * This script creates a structured JSON file with all documentaries from 
 * the old website, using the information we gathered manually.
 * 
 * Usage:
 * 1. Run: node scripts/import/manual-import.js
 */

const fs = require('fs');
const path = require('path');

// Manually curated list of documentaries from combatfilms.com/store.html
const documentaries = [
  {
...
// Format for the new store based on the existing Product interface
const formattedDocumentaries = documentaries.map(doc => ({
  id: doc.id,
  title: doc.title,
  ...
  price: doc.personalPrice, // Using personal price as default price
